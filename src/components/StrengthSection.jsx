...
const StrengthSection = () => {
  ...
  return (
    <section className=" py-16 px-4">
     <h1 className='text-2xm font-bold text-[#c59d5f] text-center p-2 mt-2' >Why Choose Us</h1>

      <div className="max-w-7xl mx-auto text-center">
        <h2 className="text-5xl text-yellow-500 font-serif mb-12">Our Strength</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {items.map((item, index) => (
            <div
              key={index}
              className="group bg-[#0f0f0f] p-8 rounded-lg shadow-md transition hover:shadow-lg hover:bg-[#181818]"
            >
              <img
                src={item.icon}
                alt={item.title}
              ...
              />
              <h3 className="text-white text-xl font-serif mb-4">{item.title}</h3>
