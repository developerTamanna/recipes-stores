...
const DiscoverRecipes = () => {
  ...
  return (
    <section className="bg-white dark:bg-black text-blue-800 dark:text-[#c59d5f] px-4 lg:px-16 py-12 text-center transition-colors duration-500">
      <h2 className="text-4xl font-semibold mb-2">
        Discover Delicious Creations
      </h2>
      <p className="max-w-xl mx-auto mb-10">
        Browse through a vibrant gallery of community‑shared recipes, each one a
        testament to culinary creativity.
      </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
        {images.map((url, index) => (
          <div
            key={index}
            className="relative rounded-lg overflow-hidden border border-blue-800 dark:border-[#c59d5f] group"
          >
            <img
              src={url}
              alt={`recipe-${index}`}
            ...
            />
            {/* Hover Shadow */}
            <div className="absolute inset-0 bg-black dark:bg-white opacity-0 group-hover:opacity-30 transition-opacity duration-300"></div>
          </div>
        ))}
      </div>
    </section>
